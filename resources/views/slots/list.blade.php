		   ...
		   @if(!empty($slots))
			...
			<li class="collection-item avatar">
				...
				<p class="blue-grey-text text-darken-4">{!! $slot["slot_desc"] !!}
					<br><br/>
					@if (Auth::user() -> role == 0)
					<a class="light-blue white-text mod-action modify link" href="{{ url('/slot/edit', (base64_encode(urlencode($slot['id'])))) }}">
						<i class="material-icons tiny relative">edit</i>Change
					</a>
					<a class="light-blue white-text margin-left-0-5x mod-action link trash" href="{{ url('/delete', (base64_encode(urlencode($slot['id'])))) }}" data-method="delete" name="delete_item">
							<i class="material-icons tiny relative">delete</i> Trash
					</a>
					<a class="light-blue white-text margin-left-0-5x mod-action link repeat" href="{{ url('/slot_repeat', (base64_encode(urlencode($slot['id'])))) }}">
