...
*/
	...
	socket.on('new_slot', function(json) {
		...
		if(json.created_by == {!! json_encode(Auth::user()->id) !!}){
					...
					+ json.end_time +"</span>"
					+(json.status  == 2 ?
					"<i class='relative material-icons green-text text-accent-4'>done</i>" : "")+"<a class='red-text text-accent-3 mod-action link cancel' href='#!' onclick='slot_cancel("+ json.slot_id + ");'><i class='material-icons tiny relative'>close</i> Cancel Request </a>"+(json.status  == 4?
